...
public record OrderReportResponse(Long orderId, String status, @JsonFormat(pattern = "dd/MM/yyyy HH:mm:ss") LocalDateTime creationDate,
    ...
    public static OrderReportResponse of(Order order) {
        var equipment = order.getEquipment();
        var client = order.getClient();

        return new OrderReportResponse(order.getId(), order.getStatus().getDescription(), order.getCreationDate(),
